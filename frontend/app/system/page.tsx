...
export default function SystemPage() {
  ...
  return (
    <BookPageShell title={UI.SYSTEM_CONSOLE_TITLE} subtitle={UI.SYSTEM_CONSOLE_SUBTITLE}>
      <div className="mb-4 flex gap-2">
        <TabButton active={tab === 'orgs'} onClick={() => setTab('orgs')} label={UI.SYSTEM_TAB_ORGS} />
        <TabButton active={tab === 'users'} onClick={() => setTab('users')} label={UI.SYSTEM_TAB_USERS} />
        <TabButton active={tab === 'features'} onClick={() => setTab('features')} label={UI.SYSTEM_TAB_FEATURES} />
      </div>
      {tab === 'orgs' ? <OrganizationSection /> : tab === 'users' ? <UserSection mode="system" /> : <SystemFeaturesSection />}
    </BookPageShell>
  );
}
