...
const FAB_ITEM_DEFS: FabItemDef[] = [
  { id: 'add', label: UI.ADD_PERSON, icon: 'plus', feature: 'editTree', contexts: ['tree'] },
  { id: 'search', label: UI.SEARCH_PERSON, icon: 'search', feature: 'search', contexts: ['tree'] },
  { id: 'book', label: UI.VIEW_GENEALOGY_BOOK, icon: 'book', feature: 'book', contexts: ['tree', 'events', 'general'], hideWhenPath: '/book' },
  { id: 'tree', label: UI.OPEN_FAMILY_TREE, icon: 'center', feature: 'tree', contexts: ['book', 'events', 'general'], hideWhenPath: '/family-tree' },
  {
    id: 'events',
    label: UI.EVENTS_FAB,
    icon: 'calendar',
  ...
  { id: 'export', label: UI.BTN_EXPORT, icon: 'image', feature: 'export', contexts: ['tree'] },
  { id: 'center', label: UI.BTN_CENTER, icon: 'center', feature: 'tree', contexts: ['tree'] },
  {
...
export function resolveFabPageContext(pathname: string, onTreePage: boolean): FabPageContext {
...
function isHiddenOnPath(def: FabItemDef, pathname: string): boolean {
  ...
  const paths = Array.isArray(def.hideWhenPath) ? def.hideWhenPath : [def.hideWhenPath];
...
export function buildFabMenuItems(options: BuildFabMenuOptions): FabMenuItem[] {
  const { context, pathname, canUseFeature, canManageCeremonyTemplates, isAdmin } = options;
  ...
  return FAB_ITEM_DEFS.filter((def) => {
    ...
    if (def.id === 'ceremonyTemplates' && !canManageCeremonyTemplates) return false;
