    ...
    .map((item) => {
      ...
      const label =
        typeof (item as { label?: unknown }).label === "string"
          ...
          : null;
      const settings = (item as { settings?: unknown }).settings as
        | Partial<TreeExportSettings>
        | undefined;
      if (!id || !label || !settings) return null;
      return {
        id,
        label,
        settings: normalizeTreeExportSettings(settings),
      } satisfies TreeExportPreset;
    })
...
export function serializeTreeExportPresets(
  presets: TreeExportPreset[],
): TreeExportPreset[] {
  return presets.map((p) => ({
    id: p.id,
    label: p.label,
    settings: normalizeTreeExportSettings(p.settings),
  }));
}
