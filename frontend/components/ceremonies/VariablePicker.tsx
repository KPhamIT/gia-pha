...
export default function VariablePicker({ variables, onInsert }: Props) {
  ...
  const countLabel = UI.CEREMONY_TEMPLATE_VARIABLES_COUNT.replace(
    "{count}",
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 min-h-0 flex-1 space-y-3 overflow-y-auto">
        {groups.length === 0 ? (
          <p className={`px-1 py-2 text-xs ${BT.mutedOnLight}`}>
            {UI.CEREMONY_TEMPLATE_VARIABLES_NONE}
          </p>
        ) : (
          groups.map(([ns, items]) => (
            <div key={ns}>
              <p className="px-1 pb-1 text-[11px] font-semibold uppercase tracking-wide text-amber-700">
                {nsLabel(ns)}
              </p>
              <div className="space-y-1">
                {items.map((item) => (
                  ...
                  >
                    <span className="min-w-0 flex-1 text-sm leading-snug text-neutral-800">
                      {item.label}
                    </span>
                    <Icon
                      path="plus"
