  ...
  return (
    <li className={`${BT.card} flex flex-col p-3 md:p-4`}>
      ...
      <div className="mt-3 flex items-center gap-2 border-t border-amber-200/60 pt-3">
        <button type="button" className={`mr-auto ${BT.btnBase} ${BT.btnSm} ${BT.btnGold}`} onClick={onPrint}>
        ...
        {isAdmin ? (
          <>
            {!template.isDefault ? (
              <IconRoundButton icon="check" variant="outline" onClick={onSetDefault} aria-label={UI.CEREMONY_TEMPLATE_USE_DEFAULT} />
            ) : null}
            <IconRoundButton icon="userPlus" variant="outline" onClick={onDuplicate} aria-label={UI.CEREMONY_TEMPLATE_DUPLICATE} />
            <IconRoundButton icon="edit" variant="outline" onClick={onEdit} aria-label={UI.BTN_EDIT} />
            <IconRoundButton icon="trash" variant="danger" onClick={onDelete} aria-label={UI.DELETE_PERSON} />
          </>
        ) : null}
      </div>
    </li>
