...
export const iconBtn = 'grid place-items-center rounded-2xl py-3 transition disabled:opacity-50';

export function ModalHeader({ title, subtitle, generation, onClose }: {
  title: string;
  subtitle: string;
  ...
  return (
    <div className="mb-5 flex items-start justify-between gap-3">
      <div>
        ...
        {generation != null ? <p className="text-sm text-slate-500">Đời thứ {generation}</p> : null}
      </div>
      <button
        type="button"
        onClick={onClose}
...
export function ActionOptions({ node, loading, onClose, onAddChild, onDelete }: {
  node: Person;
  loading: boolean;
  onClose: () => void;
  onAddChild: () => void;
  ...
  return (
    <>
      <ModalHeader title={UI.OPTIONS_FOR} subtitle={node.fullName} generation={node.generation} onClose={onClose} />
      <div className="grid grid-cols-2 gap-3">
        <button type="button" onClick={onAddChild} disabled={loading} className={`${iconBtn} bg-green-600 text-white hover:bg-green-700`}>
          <Icon path="userPlus" size={20} fill="none" stroke="currentColor" strokeWidth={2} pointer={false} />
        </button>
        <button type="button" onClick={onDelete} disabled={loading} className={`${iconBtn} bg-red-50 text-red-600 hover:bg-red-100`}>
...
export function ConfirmDelete({ node, loading, onBack, onConfirm }: {
  node: Person;
  loading: boolean;
  onBack: () => void;
  onConfirm: () => void;
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <button type="button" onClick={onConfirm} disabled={loading} className={`${iconBtn} bg-red-600 text-white hover:bg-red-700`}>
