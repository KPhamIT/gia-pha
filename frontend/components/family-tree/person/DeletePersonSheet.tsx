    ...
    <BottomSheet onClose={onClose} maxWidth="md" zClass="z-[60]">
      <div className={LAYOUT.pagePad}>
        <div className="mb-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-neutral-900">{UI.DELETE_PERSON_CONFIRM(person.fullName)}</p>
            <p className={`text-xs ${BT.mutedOnLight}`}>{UI.DELETE_IRREVERSIBLE}</p>
          </div>
        </div>
        <div className="flex justify-end gap-2">
          <IconRoundButton icon="close" variant="outline" label={UI.CANCEL} onClick={onClose} disabled={loading} />
          <IconRoundButton
            icon="trash"
            variant="danger"
