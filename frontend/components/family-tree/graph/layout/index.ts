...
export function buildFamilyTreeGraph(treeData: FamilyTreeData, config: FamilyTreeLayoutConfig = {}) {
  ...
  const levels = computeLevels(layoutRootId, childMap, parentMap, relevantPersonIds);
  ...
  const nodes = buildFlowNodes(persons, layoutRootId, coordinates, config.nodeBgColor, config.nodeTextColor, nodeWidth, nodeHeight);
