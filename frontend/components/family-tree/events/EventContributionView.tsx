...
export default function EventContributionView({
  ...
  return (
    ...
    >
      <div
        className={`overflow-hidden rounded-xl border border-amber-200/40 bg-white shadow-sm ${ET.pagePad} md:mx-6 md:mt-4`}
      >
        <div className="grid grid-cols-3 gap-px overflow-hidden rounded-lg border border-amber-300/30 bg-amber-200/50 text-center">
          ...
          <div className="bg-white py-3">
            <div className="text-lg font-bold text-slate-400">
              {livingCount - livingPaidCount}
            </div>
            <div className="text-xs text-slate-500">{UI.EVENT_UNPAID}</div>
          </div>
          <div className="bg-white py-3">
            <div className={`text-base font-bold ${ET.money}`}>
      ...
      {loading ? (
        ...
        <p className="px-4 py-12 text-center text-sm text-amber-100/70">
          {UI.EVENT_NO_MEMBERS}
        </p>
      ) : (
        <div
          className={`grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-4 ${ET.pagePad}`}
        >
          {groups.map((group) => {
            ...
            return (
              <section key={group.key} className={ET.panel}>
                <header
                  className={`flex items-center justify-between gap-2 px-3 py-2 ${ET.bandHeader}`}
                >
