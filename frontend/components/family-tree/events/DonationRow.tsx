...
export default function DonationRow({ donation, onEdit, onRemove }: Props) {
  ...
  return (
    <li className={`flex gap-3 px-4 py-3 ${isMoney ? 'items-center' : 'items-start'}`}>
      <span
        ...
        <div className={`flex justify-between gap-2 ${isMoney ? 'items-center' : 'items-start'}`}>
          <div className="min-w-0">
            ...
            {donation.kind === 'IN_KIND' && donation.itemDescription ? (
              <span className="mt-0.5 block text-sm font-semibold text-amber-900">{donation.itemDescription}</span>
            ) : null}
            {donation.note ? <span className="mt-0.5 block text-xs text-neutral-400">{donation.note}</span> : null}
          </div>
          {isMoney ? (
            <span className={`shrink-0 text-sm font-bold ${ET.money}`}>{formatDonationValue(donation)}</span>
          ) : null}
        </div>
      </div>
