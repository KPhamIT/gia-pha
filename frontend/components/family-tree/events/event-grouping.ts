...
export function groupLivingByFamily(
  ...
  const edges = normalizeParentChildEdges(
    getTreeEdges(getEffectiveRelationships(relationships)),
  );
  ...
  list.sort((a, b) => {
    const ga =
      a.head?.generation ?? a.members[0]?.generation ?? Number.MAX_SAFE_INTEGER;
    const gb =
      b.head?.generation ?? b.members[0]?.generation ?? Number.MAX_SAFE_INTEGER;
    return (
      ga - gb ||
      (a.head?.fullName ?? "").localeCompare(b.head?.fullName ?? "", "vi")
    );
  });
  return list;
}
