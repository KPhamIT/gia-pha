    ...
    <BookShell zClass="z-[60]">
      <header className={`${LAYOUT.sheetHeader} ${LAYOUT.sheetHeaderBook}`}>
        ...
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-lg font-semibold md:text-xl">{UI.BOOK_SEARCH_TITLE}</h1>
        </div>
      </header>
      ...
      <div className={`${LAYOUT.sheetBody} px-3 md:px-6`}>
        <div className="mx-auto max-w-2xl pb-[max(1rem,env(safe-area-inset-bottom))] md:pb-6">
          ...
          {query.trim() && results.length === 0 ? (
            <p className="rounded-xl bg-white/5 px-4 py-10 text-center text-sm text-amber-100/70">{UI.NO_SEARCH_RESULTS}</p>
          ) : null}
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="truncate font-medium text-amber-50">{person.fullName}</p>
                    {subtitle ? (
                      <p className="mt-0.5 line-clamp-2 text-xs leading-snug text-amber-100/60">{subtitle}</p>
                    ) : null}
                  </div>
                </button>
              );
