...
function RelationValue({ value }: { value: string }) {
  ...
  return (
    <span className={empty ? 'italic text-slate-400' : 'text-slate-800'}>{displayValue(value)}</span>
  );
}
...
export default function RelationsBlock({ relations, variant = 'stacked' }: Props) {
  ...
  return (
    <div className={styles.bookRelations}>
      ...
      {rows.map((row) => (
        <div key={row.label} className={`${styles.bookField} ${styles.bookRelationRow}`}>
