      ...
      <div className={`${LAYOUT.sidePanelOverlay} ${LAYOUT.overlayBackdropDark}`}>
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="text-base font-semibold text-amber-50 md:text-lg">{UI.SETTINGS_TITLE}</h2>
              <p className={`mt-1 text-xs ${BT.mutedOnDark}`}>{UI.SETTINGS_XY_HINT}</p>
            </div>
          ...
          <div className="mt-6 border-t border-amber-100/10 pt-4">
            {saveError ? <p className={`mb-2 text-xs ${BT.error}`}>{saveError}</p> : null}
