      ...
      <div className={`${LAYOUT.pagePad} pb-6`}>
        <div className="flex flex-col items-center text-center">
          <div className="grid h-14 w-14 place-items-center rounded-2xl bg-amber-100 text-amber-950 shadow-lg">
            <Icon path="lock" size={26} fill="none" stroke="currentColor" strokeWidth={2} pointer={false} />
          </div>
          <h2 className="mt-4 text-lg font-semibold text-neutral-900">{title}</h2>
          <p className={`mt-2 text-sm leading-relaxed ${BT.mutedOnLight}`}>{message}</p>
        </div>

        {hasContactInfo(contact) ? (
          <div className={`mt-5 rounded-2xl border border-amber-200/60 bg-amber-50/50 p-4`}>
            <p className="text-sm font-semibold text-amber-950">{UI.CONTACT_INFO_TITLE}</p>
            <p className={`mt-1 text-xs ${BT.mutedOnLight}`}>{UI.LOGIN_CONTACT_HINT}</p>
        ...
        <div className={`mt-6 flex justify-center gap-3 ${isLogin ? '' : ''}`}>
          <IconRoundButton icon="close" variant="outline" label={UI.BTN_DISMISS} onClick={close} />
          {isLogin ? (
            <IconRoundButton icon="lock" variant="primary" label={UI.BTN_LOGIN} onClick={handleLogin} />
          ) : null}
        </div>
      </div>
    </BottomSheet>
