...
export default function IconRoundButton({
  ...
  const variantClass = hasLabel
    ...
    : (ICON_ONLY_VARIANT[variant as keyof typeof ICON_ONLY_VARIANT] ??
      ICON_ONLY_VARIANT.gold);
  const alignClass =
    hasLabel && labeledAlign === "start"
      ? `justify-start ${isDense ? "gap-2" : "gap-2"}`
      : "";
  ...
  return (
    ...
    >
      {hasLabel && labeledAlign === "start" ? (
        <span className={`grid shrink-0 place-items-center ${iconSlotClass}`}>
          {iconNode}
        </span>
      ) : (
        iconNode
