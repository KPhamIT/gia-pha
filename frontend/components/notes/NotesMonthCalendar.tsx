...
/** Month grid for personal day notes (markers instead of events). */
export default function NotesMonthCalendar({
  noteDates,
  selectedDate,
  onSelectDate,
  ...
  const shiftMonth = (delta: number) => {
    const next = new Date(
      activeViewMonth.getFullYear(),
      activeViewMonth.getMonth() + delta,
      1,
    );
    setViewDate(next);
          ...
          {cells.map((cell) => {
            ...
            return (
              ...
              >
                ...
                {hasNote ? (
                  <>
                    <div className="mt-1 truncate rounded border border-[#1e3a5f]/20 bg-[#1e3a5f]/10 p-0.5 text-[10px] font-semibold text-[#1e3a5f]">
                      {UI.NOTES_MARK}
                    </div>
                    <span className="absolute bottom-1 right-1 h-1.5 w-1.5 rounded-full bg-[#1e3a5f]" />
                  </>
                ) : null}
              </button>
            );
