...
import { SYSTEM_ROLES, buildUserPayload, validateUserForm } from './user-form-utils';
...
export default function UserForm({ mode, initial, organizations, onCancel, onSubmit }: Props) {
  ...
  const handleSubmit = async () => {
    const fields = { initial, isOrgMode, username, password, email, role, organizationId };
    ...
    <div className="grid gap-3 md:grid-cols-2">
      ...
      {showRoleField ? (
        <label className="block text-sm">
          ...
          <select className={inputClassName} value={role} onChange={(e) => setRole(e.target.value as UserRole)}>
      ...
      {needsOrg ? (
        <label className="block text-sm md:col-span-2">
          ...
          <select className={inputClassName} value={organizationId} onChange={(e) => setOrganizationId(e.target.value)}>
